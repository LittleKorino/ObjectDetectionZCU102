...
// ==============================================================
`timescale 1ns/1ps
module conv_engine_control_s_axi
#(parameter
    ...
    output wire [31:0]                   use_pool,
    output wire [31:0]                   use_relu,
...
localparam
    ...
    ADDR_WEIGHTS_DATA_0      = 8'h28,
    ADDR_WEIGHTS_DATA_1      = 8'h2c,
    ADDR_WEIGHTS_CTRL        = 8'h30,
    ADDR_BN_PARAMS_DATA_0    = 8'h34,
    ADDR_BN_PARAMS_DATA_1    = 8'h38,
    ADDR_BN_PARAMS_CTRL      = 8'h3c,
    ADDR_IN_CHANNELS_DATA_0  = 8'h40,
    ADDR_IN_CHANNELS_CTRL    = 8'h44,
    ADDR_OUT_CHANNELS_DATA_0 = 8'h48,
    ADDR_OUT_CHANNELS_CTRL   = 8'h4c,
    ADDR_IN_HEIGHT_DATA_0    = 8'h50,
    ADDR_IN_HEIGHT_CTRL      = 8'h54,
    ADDR_IN_WIDTH_DATA_0     = 8'h58,
    ADDR_IN_WIDTH_CTRL       = 8'h5c,
    ADDR_KERNEL_SIZE_DATA_0  = 8'h60,
    ADDR_KERNEL_SIZE_CTRL    = 8'h64,
    ADDR_STRIDE_DATA_0       = 8'h68,
    ADDR_STRIDE_CTRL         = 8'h6c,
    ADDR_PADDING_DATA_0      = 8'h70,
    ADDR_PADDING_CTRL        = 8'h74,
    ADDR_USE_POOL_DATA_0     = 8'h78,
    ADDR_USE_POOL_CTRL       = 8'h7c,
    ADDR_USE_RELU_DATA_0     = 8'h80,
    ADDR_USE_RELU_CTRL       = 8'h84,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_use_pool = 'b0;
    reg  [31:0]                   int_use_relu = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BN_PARAMS_DATA_0: begin
                    rdata <= int_bn_params[31:0];
                end
                ADDR_BN_PARAMS_DATA_1: begin
                    rdata <= int_bn_params[63:32];
                end
                ...
                ADDR_USE_POOL_DATA_0: begin
                    rdata <= int_use_pool[31:0];
                end
                ADDR_USE_RELU_DATA_0: begin
                    rdata <= int_use_relu[31:0];
                end
            endcase
        end
    end
...
assign bn_params         = int_bn_params;
...
assign use_pool          = int_use_pool;
assign use_relu          = int_use_relu;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_bn_params[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BN_PARAMS_DATA_0)
            int_bn_params[31:0] <= (WDATA[31:0] & wmask) | (int_bn_params[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_bn_params[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BN_PARAMS_DATA_1)
            int_bn_params[63:32] <= (WDATA[31:0] & wmask) | (int_bn_params[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_USE_POOL_DATA_0)
            int_use_pool[31:0] <= (WDATA[31:0] & wmask) | (int_use_pool[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_USE_RELU_DATA_0)
            int_use_relu[31:0] <= (WDATA[31:0] & wmask) | (int_use_relu[31:0] & ~wmask);
    end
end
